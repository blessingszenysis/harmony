...
def main():
    ...
    # Take the first mapping from CBO94 -> CBO2002 since the code requires only one line
    # per CBO code.
    cbo_94 = cbo_94.groupby('CBO94').first().reset_index()
    cbo_94 = (
        cbo_94.merge(
            cbo_titles,
            left_on='CBO2002',
    ...
    full_cbo = pd.concat([short_titles, cbo_94, cbo_titles])
    assert len(full_cbo) == len(
        full_cbo[CODE_COLUMN].unique()
    ), 'Full CBO mapping contains duplicate ID codes'

    LOG.info('Outputting merged file')
    full_cbo.to_csv(Flags.ARGS.output, index=False)
